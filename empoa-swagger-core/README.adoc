...
An other approach, if you have an existing instance of Swagger-Core models (for example the ouput of Swagger-Parser), `fr.jmini.empoa.swagger.core.internal.SwAdapter` can be used as proposed in <<lst-swagger-core-usage>>.

[[lst-swagger-core-usage]]
[source, java, indent=0]
.Example usages of {module-name}
----
include::src/test/java/fr/jmini/empoa/swagger/core/tck/ParserAndSerializerWithGsonTckTest.java[tags="usage"]
----

If you are looking for factory methods where no Swagger project elements are part of the method signature, you can use `fr.jmini.empoa.swagger.core.SwOASFactory`. 
